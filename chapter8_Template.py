...
class QueryTemplate:
    ...
    def format_results(self):
        ...
        for row in self.results:
            #print('1.', row) #row is tuple of different types
            row = [str(i) for i in row]
            #print('1.', row) #row is list of strings
...
class UserGrossQuery(QueryTemplate):
    ...
    def output_results(self):
        filename = "chapter8_Template_output_{0}.txt".format(datetime.date.today().strftime("%Y%m%d"))
